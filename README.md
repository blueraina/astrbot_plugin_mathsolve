...
> 外部联网搜索与常规知识库工具调用可交给 AstrBot 自身的 Agent / 工具系统处理。本插件不再内置联网搜索关键词配置；插件知识库增强默认关闭。
...
- 默认关闭，避免和 AstrBot Agent 自身的知识库工具调度重复
- 开启后识别“从知识库/题库找类似题”等意图
- 调用 AstrBot 本地知识库工具 `astr_kb_search` 检索
...
- 可按会话归档完整图文答疑、`/pdf` 和 `/spdf` 的题目与解答
- `/pdf`、`/spdf` 会额外保存对应 TeX 源码，便于日报整理
- 支持自动提前生成 LaTeX 讲义 PDF，到设定时间发送
- 支持 `/答疑报告生成` 手动生成当前会话区间报告，适合测试日报效果
- 报告默认输出知识地图、专题讲义、高频误区和典型题精讲；典型题会按题目拆成独立卡片
